...
// src/helpers/Suggester.ts
var import_obsidian = __toModule(require("obsidian"));
var SuggesterModal = class extends import_obsidian.FuzzySuggestModal {
  constructor(app, textItems, items, resolve, reject, placeholder, limit) {
    super(app);
...
function suggest(app, textItems, items, placeholder, limit) {
  return __async(this, null, function* () {
    return new Promise((resolve, reject) => {
      new SuggesterModal(app, textItems, items, resolve, reject, placeholder, limit).open();
    });
  });
}
...
var Helpers = class {
  ...
  suggest(textItems, items, placeholder, limit) {
...
// src/settings/utils/Utils.ts
var import_obsidian4 = __toModule(require("obsidian"));

// src/settings/utils/Error.ts
var UserPluginError = class extends Error {
};
...
var SettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  addUserScriptsListSetting() {
    if (!this.plugin.settings.scriptsFolder) {
      new import_obsidian5.Setting(this.containerEl).setName("No user scripts folder set").addExtraButton((extra) => {
    ...
    let files;
    try {
      files = get_tfiles_from_folder(this.app, this.plugin.settings.scriptsFolder);
    } catch (e) {
    ...
    new import_obsidian5.Setting(this.containerEl).setName(name).addExtraButton((extra) => {
      extra.setIcon("sync").setTooltip("Refresh").onClick(() => {
        this.display();
      });
    });
  }
  addUserScriptEntries(files) {
    let added = 0;
    const enabled = this.plugin.settings.enabledScripts;
    const absentFilesToRemove = [...enabled];
    for (const file of files) {
      if (file.extension === "js") {
        new import_obsidian5.Setting(this.containerEl).setName(file.name).setDesc("Enable this script").addToggle((cb) => {
          cb.setValue(enabled.includes(file.path)).onChange((newValue) => {
  ...
  addSnippetsSetting() {
    ...
    this.plugin.settings.snippets.forEach((_, idx) => {
      const setting2 = new import_obsidian5.Setting(this.containerEl).addExtraButton((extra) => {
        extra.setIcon("right-arrow").setTooltip("Run snippet").onClick(() => {
          const script = scripts[idx];
          this.plugin.runSnippet(script, idx);
        });
      }).addTextArea((text) => {
        const t = text.setPlaceholder("User script").setValue(scripts[idx]).onChange((new_script) => {
          const script = scripts[idx];
          const index = this.plugin.settings.snippets.indexOf(script);
          if (index > -1) {
            this.plugin.settings.snippets[index] = new_script;
            this.plugin.saveSettings();
          }
        });
    ...
    const setting = new import_obsidian5.Setting(this.containerEl).addButton((button) => {
      button.setButtonText("Add new user script").setCta().onClick(() => {
  ...
  addScriptFolderSetting() {
    this.containerEl.createEl("h2", { text: "Scripts" });
    new import_obsidian5.Setting(this.containerEl).setName("Scripts folder location").setDesc("Files in this folder can be run on Obsidian startup").addSearch((cb) => {
      new FolderSuggester_default(this.app, cb.inputEl);
      cb.setPlaceholder("Ex.: Folder/MyScripts").setValue(this.plugin.settings.scriptsFolder).onChange((newFolder) => {
...
var UserPlugins = class extends import_obsidian6.Plugin {
  constructor() {
    super(...arguments);
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      this.app.workspace.onLayoutReady(() => __async(this, null, function* () {
        try {
          yield this.runScripts();
          yield this.runSnippets();
        } catch (e) {
  ...
  runSnippet(snippet, idx) {
    return __async(this, null, function* () {
      try {
        Function("plugin", snippet)(this);
      } catch (e) {
        Error.captureStackTrace(e);
        this.logScriptError(`Error running snippet no.${idx}: ${e.message}`, e);
      }
    });
  }
  runSnippets() {
    return __async(this, null, function* () {
      ...
      for (const script of this.settings.snippets) {
        yield this.runSnippet(script, count);
        count++;
      }
    });
  }
  runScript(path) {
    return __async(this, null, function* () {
      ...
      const userModule = {};
      try {
        Function("module", yield this.app.vault.read(file))(userModule);
      } catch (e) {
        Error.captureStackTrace(e);
        this.logScriptError(`${file.path} evaluation error: ${e.message}`, e);
        return;
      }
      try {
        if ("onload" in userModule.exports) {
          yield userModule.exports.onload(this);
        }
      } catch (e) {
        Error.captureStackTrace(e);
  ...
  runOnunload(path) {
    return __async(this, null, function* () {
      const toRemove = [];
      yield Promise.all(this.modulesWithUnload.filter(([p, _]) => p === path).map((_0, _1) => __async(this, [_0, _1], function* ([path2, userModule], idx) {
        yield this.runOnUnloadOfUserModule(path2, userModule);
        toRemove.push(idx);
      })));
      toRemove.slice().reverse().forEach((idx) => this.modulesWithUnload.splice(idx, 1));
    });
  }
  runScripts() {
    return __async(this, null, function* () {
      for (const path of this.settings.enabledScripts) {
        yield this.runScript(path);
      }
    });
  }
  logScriptError(msg, error) {
    this.logError(`${msg}. stacktrace: ${error.stack}`);
  }
  logError(msg) {
    console.error(`[obsidian-user-plugins] ${msg}`);
  }
  runOnUnloadOfUserModule(path, userModule) {
    return __async(this, null, function* () {
      try {
        yield userModule.exports.onunload(this);
      } catch (e) {
        Error.captureStackTrace(e);
        this.logScriptError(`Error running ${path}.onunload: ${e.message}`, e);
      }
    });
  }
  onunload() {
    return __async(this, null, function* () {
      for (const [path, userModule] of this.modulesWithUnload) {
        yield this.runOnUnloadOfUserModule(path, userModule);
      }
    });
  }
  loadSettings() {
